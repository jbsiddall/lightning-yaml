...
<!-- BENCH:OURS:START -->
_`lightning-yaml` is registered as a candidate but its `parse`/`stringify` are not implemented yet (they throw `NotImplementedError`), so there is nothing to benchmark here._

_This block refreshes automatically once `src/index.ts` implements them; until then, `pnpm test` runs the consistency suite that specifies what "correct" means._
<!-- BENCH:OURS:END -->
