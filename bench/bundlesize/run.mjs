...
async function main() {
  ...
  const { BUNDLERS } = await import("./bundlers.mjs");

  // Partition bundlers into available vs skipped (report the skips once).
  const active = [];
  const skipped = [];
  for (const b of BUNDLERS) {
    if (b.available()) active.push(b);
  ...
  const md = renderMarkdown(results, active, skipped);
  console.log("\n" + md + "\n");
...
function renderMarkdown(results, active, skipped) {
  ...
  const activeNote = active
    .map((b) => (b.rust ? `${b.name} (rust)` : b.name))
    .join(", ");
