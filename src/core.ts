 ...
 */
function keyToString(node: unknown, visited?: Set<object>): string {
  if (typeof node === "string") return node;
  if (node === null) return "";
  if (typeof node === "object") return stringifyKeyNode(node as object, visited);
  return String(node);
}
...
function stringifyKeyNode(node: object, visited = new Set<object>()): string {
  ...
  try {
    ...
    if (node instanceof Map) {
      ...
      return stringifyKeyItems(entries.length, (i) => `${stringifyKeyScalar(keyToString(entries[i]![0], visited))}: ${stringifyKeyValue(entries[i]![1], visited)}`, "{", "}");
    }
